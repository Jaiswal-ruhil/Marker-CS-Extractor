...
def process(
    ...
    df_merged.insert(0, "Sr No", range(1, len(df_merged) + 1))
...
class NumberedCanvas(canvas.Canvas):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.generation_time: str = datetime.datetime.now().strftime("%d-%b-%Y %H:%M:%S IST")
    ...
    def draw_page_decorations(self, num_pages: int) -> None:
        ...
        self.drawRightString(792, 25, self.generation_time)
...
def save_as_pdf_v02(
    ...
    col_widths = [40, 130, 330, 80, 80, 97]
    prod_header_row = [
        Paragraph("<b>Sr No</b>", meta_label_style),
    ...
    for r in df_merged.itertuples(index=False):
        prod_data_rows.append([
            ...
            Paragraph(str(r[3]), meta_val_style),
            Paragraph(f"{r[4]:.2f}" if pd.notna(r[4]) else "", meta_val_style),
            Paragraph(str(int(r[5])) if r[5].is_integer() else f"{r[5]:.2f}", meta_val_style),
        ])
